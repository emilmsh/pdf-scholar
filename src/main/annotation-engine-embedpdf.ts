...
const toRect = (r: PageRect): { origin: { x: number; y: number }; size: { width: number; height: number } } => ({
  origin: { x: r.x, y: r.y },
  size: { width: r.w, height: r.h }
})

/** Bounding box of a set of strokes, padded — PDFium wants a rect supplied */
function strokesBBox(strokes: [number, number][][], pad: number): PageRect {
  let x0 = Infinity
  let y0 = Infinity
  let x1 = -Infinity
  let y1 = -Infinity
...
const MARKUP = new Set<AnnotateRequest['type']>(['highlight', 'underline', 'strikeout', 'squiggly'])

function buildAnnotation(req: AnnotateRequest): PdfAnnotationObject | { error: string } {
  const base = {
    id: randomUUID(),
