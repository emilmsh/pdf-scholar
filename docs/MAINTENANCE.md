...
| 4 | **Service base URLs, local presets + key-console links** (`COMPAT_SERVICES` in `src/shared/ai-provider-profile.ts`; `LOCAL_SERVICES` — the local servers' default URLs + example model-id placeholders — in the same file; `KEY_CONSOLE_URLS`/`KEY_CONSOLE_LABELS` in `src/renderer/src/components/ai-models.ts`) | Do the URLs still resolve, and do the example ids still exist? | Update the constants — no other surface mirrors them. A moved service base URL breaks every user of that provider, so this row is the audit's most load-bearing |
