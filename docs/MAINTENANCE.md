...
| 2 | **Degrade-net firings** (a heuristic quietly wrong: answers lose thinking tuning / modern search) | `npm run test:live` — it counts the HTTP requests per question, so a retried 400 is a failed check rather than something you have to spot in devtools. Covers the rest of the contract in the same run (citations survive, images read or named, nothing silently stuck) | [docs/AI-TESTING.md](AI-TESTING.md); a stream worth keeping goes into the replay library with `-- --record` |
