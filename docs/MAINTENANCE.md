...
| 4 | **Compat preset URLs + spend-cap links** (`COMPAT_PRESETS`, `SPEND_CAP_URLS` in `src/renderer/src/components/ai-models.ts`) | Do the URLs still resolve to the right pages? | Update the constants — no other surface mirrors them |
