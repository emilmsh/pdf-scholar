...
- PdfPage: prop `selectedId` → seleksjonsramme (bbox + hjørnemarkører) i egen pointer-events:none-overlay (også for file-annots).
...
- **Oppdatert 2026-07-29 — «ingen resize i v1» gjelder ikke lenger.** Hjørnemarkørene er interaktive håndtak (firkant/ellipse/fritekst/blekk), linje/pil har endepunkthåndtak, og tekstmarkeringer — som denne notatet riktig kalte «ikke flyttbare» — redigeres i stedet ved å dra endene, i tekstrom med ordsnapping (`text-range.ts`). Motorbehovet over ble løst som `quads`/`strokes` på `ModifyAnnotationRequest` (ny geometri, ikke en delta), implementert i begge skrivestier og dekket av `test:engine` + `test:appender`. Fortsatt ikke bygget: trykkfølsom penn, lasso-multivalg, og håndtak under rotasjon (samme grense som tegneverktøyene).
