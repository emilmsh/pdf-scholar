...
- [x] Tekststørrelse i assistenten (levert 2026-08-10, brukerønske): én skala i faste trinn (80–160 %, standard 100 %) for samtaleflaten — meldinger, komponist, landing, forklar-boblen; header/historikk/innstillinger er krom og står stille. Betjenes fra en rad i modellmenyen (− / % / + / Nullstill) og med Ctrl+rullehjul over panelet (dokumentzoom-lytteren sitter på `.pages`, så gesten var ledig); Ctrl+± forblir dokumentzoom. `--ai-scale`-var på panelroten, `calc()`-multiplikasjon i CSS-en (100 % er piksel-identisk med før), lagret i localStorage (`ai-text-scale.ts`, samme mønster som panelbredder) — identisk på alle plattformer av konstruksjon
- [x] Assistenten i eget vindu (levert 2026-08-10, brukerønske): «Åpne i eget vindu»-knapp i panelhodet → chatten for dokumentet i eget rammeløst vindu (desktop; egne husket-bounds under `state.assistantWindow`) eller egen fane (utvidelsen/dev:web) — skallet er `AssistantApp.tsx`, som leser de samme bytene (utkast-bevisst) og trekker ut tekst/sidebilder fra sitt eget pdf.js-håndtak uten å montere en eneste side. Sitatklikk reléres til vinduet som VISER dokumentet (main-IPC på desktop med vindusheving; BroadcastChannel + faneselvaktivering ellers), med «Åpne dokumentet»-toast når ingen har det. Samtalene deler samme lager som det dokkede panelet (storage-event-refresh); snip og annotasjonschip finnes bare dokket (krever montert viewer). Divergensene ført i `docs/PLATFORMS.md` pkt. 17; dekket ende-til-ende av `npm run test:assistant`. Samtidig herdet: in-flight AI-kall i main nøkles nå per vindu (`sender.id:requestId`) — to vinduer kunne minte samme id og avbryte hverandres svar
...
- **Zotero-integrasjon** (Emils notat 2026-07-12; researchet og nivå 1 bygget 2026-09-02 etter en direkte forespørsel på LinkedIn): Nivå 0–1 er levert — ekstern-leser-workflowen dokumentert (README «Working with Zotero»; annoteringene våre er standard annots som Zoteros «Import Annotations…» adopterer) og Zotero-bevissthet i appen (`shared/zotero.ts` + `main/zotero.ts`: sti → attachment-key, lokal-API-oppslag, «Vis i Zotero», kopier sitering/referanse i lagre-menyen; PLATFORMS §18, `test:zotero`). **Parkerte, researchede spor:** *Nivå 2* — «Lagre til Zotero» i extensionen via connector-endepunktene (`POST 127.0.0.1:23119/connector/saveItems`); undersøk samtidig om viewer-redirecten vår ødelegger Zotero Connector-utvidelsens egen PDF-gjenkjenning (i så fall paritets-fix, ikke bare feature). *Nivå 3* — toveis skriving via Zotero 10+ sitt lokale write-API (bruker-godkjent API-nøkkel + `Zotero-Server-ID`-header): push annotasjoner/AI-notater som native Zotero-objekter, forbi importbegrensningene (FreeText/Ink) — uavklart om API-skapte annotasjoner vises som eksterne/read-only i Zotero-leseren. Better BibTeX CAYW-picker (`/better-bibtex/cayw`) står også urørt. **DOI-reserve (bygget 2026-09-07)** etter en lesers tips om RefRunner (DOI-basert formatterer, tredjepart med kvote og betalingsmur — avvist som integrasjon): dokumenter utenfor Zotero med DOI på de første sidene/i metadata får en «Referanse»-seksjon i lagre-menyen som på klikk henter APA-referanse og BibTeX rett fra doi.org (Crossref/DataCite content negotiation, `shared/doi.ts`, `test:doi`). Zotero-posten vinner når den finnes. **Stilvalg bygget samme dag:** én innstilling (`citationStyle`) for begge kilder, åtte kuraterte CSL-stiler som Zotero 7 leverer installert; in-text for DOI-siterte dokumenter avledes per stilfamilie, numeriske/note-stiler gir ingen. *Mulig neste:* RIS-eksport (`application/x-research-info-systems`), og å tilby DOI-oppslaget også når Zotero-posten mangler metadata.
- **Legg monetiseringsstrategi**: hvordan PDFX eventuelt kan tjene penger (lisens/kjøp/abonnement/gratis+pro — omfang avklares med Emil når det tas opp). (Emils ønske 2026-07-09. **Lagt bort 2026-09-02**, i forbindelse med Zotero-researchen: gratis + MIT + Sponsors står — integrasjoner som Zotero bygger adopsjon, ikke betalingsmur, og alt vi bygger er lokalt med null løpende kostnad. Tas ev. opp igjen som eget regnestykke når noe faktisk koster oss noe løpende.)
