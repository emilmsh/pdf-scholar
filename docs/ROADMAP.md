...
- [ ] **Fase 1 — generisk OpenAI-kompatibel leverandør** (størst gevinst per linje): base-URL + valgfri nøkkel + modell-id, gjenbruker Azure-stien (`chatOpenAiCompatible`). Preset-liste som bare forhåndsutfyller base-URL: OpenRouter, Mistral, Groq, xAI — og **lokale modeller via Ollama og LM Studio**, som begge serverer `/v1/chat/completions`. Nøkkel valgfri (lokale servere har ingen — `keyRequired: false`). `GET /v1/models` for kataloglisting der serveren støtter det.
- [ ] **Fase 2 — Ollama førsteklasses lokal leverandør**: auto-deteksjon av server, modelliste fra `/api/tags`, og reell kontekstlengde + vision-støtte per modell fra `/api/show` — lokale modeller har ofte 8–32k kontekst, så BM25-utdragsmodusen (`ai-retrieval.ts`) blir hovedvei og gulvet bør være presist, ikke gjettet. Ingen nøkkel, ingen kostnadspåminnelse, «lokal»-merke i modellmenyen.
- [ ] **Fase 3 — Gemini med eget API**: håndrullet fetch (ingen ny SDK, samme regel som OpenAI-stien), Google Search-grounding som nettsøk-verktøy, 1M kontekst. (Kan nås via sitt OpenAI-kompatible endepunkt allerede i fase 1; eget løp er oppgraderingen.)
