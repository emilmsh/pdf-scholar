...
|---|---|---|
| Live catalog (`src/shared/ai-model-catalog.ts`) | Fetches `GET /v1/models` from Anthropic (with capability data), OpenAI (ids only) and each keyed hosted service — OpenRouter/Gemini/xAI/Mistral/Groq (ids, plus name + `context_length` where the listing carries them) — cached 24 h in `pdfx-state.json` / `chrome.storage.local`; a custom compat endpoint is fetched the same way (5 min TTL when local, keyed to its base URL, Ollama-enriched). New models appear in the model menu on their own; retired ones get a ⚠ marker. | Labels/hints stay generic (`prettyModelName`); OpenAI capabilities; Azure (manual by design); the compat family has no curated lists at all (nothing here to maintain beyond base URLs/presets, see docs/MAINTENANCE.md row 4); pricing. |
