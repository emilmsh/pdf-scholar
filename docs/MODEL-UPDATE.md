...
|---|---|---|
| Live catalog (`src/shared/ai-model-catalog.ts`) | Fetches `GET /v1/models` from Anthropic (with capability data) and OpenAI (ids only) with the user's key, cached 24 h in `pdfx-state.json` / `chrome.storage.local`; the compat provider's endpoint is fetched the same way (ids only, 5 min TTL, keyed to its base URL). New models appear in the model menu on their own; retired ones get a ⚠ marker. | Labels/hints stay generic (`prettyModelName`); OpenAI capabilities; Azure (manual by design); compat has no curated list at all (per-endpoint by nature — nothing here to maintain beyond the presets, see docs/MAINTENANCE.md row 4); pricing. |
