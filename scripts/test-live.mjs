// Conformance run against the REAL providers, with real keys and real money.
//
...
//
// Run:
//   node scripts/test-live.mjs --self-check     no keys, local fake endpoint
//   node scripts/test-live.mjs                  every provider with a key in env
//   node scripts/test-live.mjs --provider=openrouter --model=moonshotai/kimi-k2.5
//   node scripts/test-live.mjs --record         also save each stream for replay
//
...
import { curatedIds, instrumentFetch, loadAiCore, saveRecording } from './lib/ai-core.mjs'
import { startFakeProvider, SCENARIOS } from './lib/fake-provider.mjs'
import { encodePng } from './lib/tiny-png.mjs'
...
const keyFor = (provider) => process.env[KEY_ENV[provider]] ?? ''
...
if (SELF_CHECK) {
  ...
  if (providers.length === 0) {
    console.log('No provider keys in env — nothing to ask.')
    console.log(`Set any of: ${Object.values(KEY_ENV).join(', ')}`)
    console.log('Or run `node scripts/test-live.mjs --self-check` for the keyless harness check.')
    process.exit(0)
  }
